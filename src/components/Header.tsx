        ...
        <div className={`flex items-center justify-between transition-all duration-700 px-6 sm:px-10 py-4 ${
          ...
          <Link to="/" className="flex items-center space-x-3 group">
            ...
            <div className="flex flex-col">
              <span className={`text-2xl font-black tracking-tight transition-colors duration-700 ${isScrolled ? 'text-white' : 'text-slate-900'}`}>
          ...
          <nav className="hidden lg:flex items-center space-x-10">
            ...
            <div className="ml-6">
              <button 
                ...
                className={`font-black py-3 px-8 rounded-2xl transition-all duration-700 flex items-center space-x-3 text-xs uppercase tracking-widest shadow-2xl ${
                  isScrolled 
          ...
          <div className="lg:hidden mt-4 py-4 bg-white/95 backdrop-blur-xl rounded-2xl border border-gray-100 shadow-2xl animate-slide-in-up">
            <nav className="flex flex-col space-y-2 px-4">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.name}
                    to={item.href}
                    className={`px-4 py-3 rounded-xl font-medium transition-colors duration-300 flex items-center space-x-3 ${
                      isActive 
                        ? 'bg-taxi-yellow/10 text-taxi-black font-bold' 
              ...
              <button 
                ...
                className="mt-4 bg-taxi-yellow text-taxi-black font-bold py-3 px-6 rounded-xl hover:bg-yellow-400 transition-colors duration-300 flex items-center justify-center space-x-2"
              >
